    ...
    public function edit($id){
        ...
        public function productListFrontView(Request $request, $main_category, $sub_category = ''){
           try{ 
            ...
            if($sub_category != ''){
            $sub_cat_id = SubCategory::where('slug', $sub_category)->first();
            $sub_cat_name = $sub_cat_id->name;
            if($filter != 0){
                $product_list = $product_list->whereJsonContains('sub_category', $filter);
            }else{
                $product_list = $product_list->whereJsonContains('sub_category', $sub_cat_id->id);
            }
            }
            if($sub_category == ''){
            ...
            'main_category', 'sub_category', 'sub_cat_list', 'page_url', 'main_category_name', 'sub_cat_name')); 
        }catch(\Exception $e){
            abort('404');
        }
        }
